...
public class ColetaLixo extends Thread {
    ...
    public void separarLixos() {
        ...
        if (lixoColetado != null) {
            ...
            System.out.println("Coleta separou: " + lixoColetado + (lixoColetado.eReciclavel() ? "RECICLAVEL" : "NÃO RECICLAVEL"));
        } else {
            try {
                Thread.sleep(1000 * 2);
    ...
    public void mostraResultado() {
        ...
        System.out.println("Coleta separou: " + reciclavel.size());
        System.out.println("Coleta separou: " + naoReciclavel.size());
        int total = reciclavel.size() + naoReciclavel.size();
        if (total > 0) {
            double perc = ((double) reciclavel.size() / total) * 100;
            String sustentavel = perc >= 50.0 ? "SUSTENTÁVEL (>= 50%)" : "NÃO SUSTENTÁVEL (< 50%)";
